...
As usual, we ignore NA values with `na.rm=TRUE`, but this time we want to count the number of NA values and include this number in some plot text. We count the NAs by "pulling" the `bmi0` variable from the data, checking which observations have are NA, and summing these up. Alternatively, we could have done this using the `summarize()` function.
...
Next we'll make a scatterplot of `bmi0` against `bmi1`, and include the regression line. We can draw a line in `ggplot2` with the `geom_abline()` function, supplying the intercept and the slope of the line. The intercept and slope of the line are our coefficients from the linear model above, where the slope corresponds to our `bmi0` estimate. We see there are some outliers that the model fits poorly, but in general the fit seems intuitively sensible.
...
ggplot(data, aes(bmi0, bmi1)) +
  ...
  labs(title="Regression line of linear model", x='BMI at birth', y='BMI at exam day 1')
```

The last thing we want to do before we wrap this case study up is do a little bit of model inspection by plotting the residuals. We assume that the error is normally distributed with zero mean, $\epsilon \in N(0, \sigma_{\epsilon})$. If this assumption is violated, then the model is a poor fit on the data, regardless of how impressive our p-values are.
...
Below, we plot the residuals with a boxplot, a qq-plot and a histogram. This time, we have made three plots in one window, using the `multiplot()` function from the `multiplot.R` script we downloaded. We get the handle of each plot as `p1`, `p2` and `p3`, and then specify that we want the plots in three columns (in this case this puts all three plots in one row). We've also added the `tag` label to the plots.

The data is so obscured by the outliers that it is difficult to say whether it is reasonable to accept them as normally distributed. Therefore, we will try to deal with these outliers in our next case study.
